...
/**
 * `work.committed: false` is D36's one knob — the choice for a fork of a stranger's
 * repository, where `brief.md` and `plan.md` in a pull request are noise — and nothing read
 * it: the work directory stayed untracked rather than ignored, one `git add` from the PR.
 */
export function ignoredLines(config) {
  return config?.work?.committed === false ? [TMP_IGNORE, WORK_IGNORE] : [TMP_IGNORE];
}

export function ensureIgnored(root, config) {
...
export function applyInit(root, config) {
  ...
  for (const target of initTargets(root)) {
    if (target.kind === "gitignore") record(target, ensureIgnored(root, config));
