...
function stackIn(dir, label) {
  const pkg = readJsonIfPresent(join(dir, "package.json"));
  if (pkg) return { id: "node", cmd: nodeCommands(pkg, dir), evidence: `${label}package.json` };

  const composer = readJsonIfPresent(join(dir, "composer.json"));
  if (composer) {
    const isCi3 = JSON.stringify(composer.require ?? {}).includes("codeigniter");
    return { id: isCi3 ? "php-ci3" : "php", cmd: {}, evidence: `${label}composer.json` };
  }
  return null;
}

/**
 * A superproject often holds no manifest of its own — every one lives in a submodule. Read
 * only at the root, detection answered "unknown" for a repository with four `composer.json`
 * files in it, and the user then had to name the stack by hand on a project kiln could see
 * perfectly well.
 */
export function detectStack(root) {
  const here = stackIn(root, "");
  if (here) return here;

  for (const [, path] of Object.entries(detectModules(root))) {
    const found = stackIn(join(root, path), `${path}/`);
