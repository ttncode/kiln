...
function existingPrefix(path) {
  ...
  for (;;) {
    try {
      ...
      // fail-closed: a missing directory is climbed past, never read as "inside".
...
export function isSymlink(path) {
  try {
    ...
    // fail-closed: a path that cannot be stat'ed is resolved through its parent, which must
    // then exist inside the root for anything to be allowed.
    return false;
  }
}
...
/**
 * Where a name lives, without following it. Removing or moving a symlink acts on the link,
 * never on what it points at — so `rm link.md` has nothing to refuse, and resolving it the
 * way a write is resolved reported a deliberate refusal as a bug in kiln.
 */
export function resolveEntry(path, cwd) {
  const absolute = isAbsolute(path) ? resolve(path) : resolve(cwd, path);
  const { real, climbed } = existingPrefix(dirname(absolute));
  return join(real, ...climbed, basename(absolute));
}
...
export function targetIsInside(root, { path, cwd }) {
  try {
    ...
    // fail-closed: unresolvable is outside.
    return false;
  }
}
