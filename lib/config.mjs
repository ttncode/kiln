...
/**
 * tmp + rename, as state is written. A config the guards cannot read now blocks every
 * write, so a half-written one — a crash mid-`config set` — would brick the session it was
 * meant to adjust.
 */
export function writeConfig(root, config) {
  const target = configPath(root);
  writeFileSync(`${target}.tmp`, `${JSON.stringify(config, null, 2)}\n`, "utf8");
