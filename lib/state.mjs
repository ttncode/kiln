...
/** D188: which practices a stage was handed, one entry per stage, replaced like the rules record. */
export function recordPractices(state, { stage, ids }) {
  const others = (state.practices ?? []).filter((row) => row.stage !== stage);
  return { ...state, practices: [...others, { stage, ids }] };
}
 ...
 *
 * The range is provenance, not the test. SHIP commits after VERIFY by design, which moves
 * HEAD and changes the range while the content stays byte for byte what was verified — and
 * comparing ranges called every shipped run stale, the warning that is always wrong (D89).
 */
export function isStaleVerify(entry, current) {
  if (!entry.tree || !current.tree) return true;
  return entry.tree !== current.tree;
}
