...
function checkTrigger(root, trigger) {
  ...
  if (!paths.some((path) => match.test(normalise(path)))) {
...
export function addRoute(root, { file, trigger, text }) {
  checkName(file);
  checkTrigger(root, trigger);
  ...
  if (!existsSync(path)) {
    ...
    wrote.push(`created .kiln/rules/${file}`);
  } else if (text) {
    refuse(`.kiln/rules/${file} already exists. This verb adds rules; it never rewrites one — edit the file yourself, or route a new one.`);
  }

  const routed = readRoutes(root).some((row) => row.file === file && row.trigger === trigger);
  if (routed) return [...wrote, `already routed: ${trigger} → ${file}`];

  const index = indexPath(root);
  const before = existsSync(index) ? readFileSync(index, "utf8") : "# Project rules\n";
  writeFileSync(index, withRow(before, tableRow(trigger, file)), "utf8");
  return [...wrote, `routed ${trigger} → ${file}`];
}
