...
const NO_PATHS = {
  plan: "No files were named, so there is nothing to match a rule against.\nPass the change preview's paths: `kiln rules <id> --stage plan --predicted a.php,b.php`.\nThe approved claim in state.predicted is only written at the gate, which is after this.",
...
function trackedPaths(root) {
  const paths = trackedUnder(root).map(normalise);
...
/**
 * Asking for the same rule twice is not an error, and the natural call carries `--text`.
 *
 * Measured on the owner's second validation run: they asked for the same rule again, and the
 * agent checked the file itself and skipped the verb rather than calling it — which was the
 * right move, because the verb would have refused. An agent routing around a verb is the same
 * failure as an agent routing around a guard, one layer up.
 *
 * So the text decides. Identical to what is there: nothing to do, say so. Different: refused,
 * because add-only is what keeps D102 intact and rewriting a rule is exactly what a run must
 * not be able to do. The refusal now says *which* of the two it is, so it cannot be read as
 * spurious.
 */
function writeRule(root, { file, text }) {
  ...
  if (!text) return [`.kiln/rules/${file} is already there, unchanged`];
  if (readFileSync(path, "utf8").trimEnd() === text.trimEnd()) {
    return [`.kiln/rules/${file} already says exactly this — nothing to write`];
  }
  return refuse(`.kiln/rules/${file} already exists and its content differs from what you passed.
This verb adds rules; it never rewrites one, because a rule is one of the terms a run is judged by.
Edit the file yourself, or route a new rule beside it.`);
}
