 ...
 * neither is a pass that ran nothing at all. `{entries: [], failed: null}` is the shape an
 * empty step list produces, and reading it as green is D7 item 3 reached through config
 * rather than through a lying test.
 */
export function isGreen(result, phase) {
  return phase === "full" && result.failed === null && result.entries.length > 0;
}
