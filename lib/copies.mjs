...
/** A markdown copy carries kiln's note; a script or source file cannot, so its whole text is its body. */
...
function textProblems(entry, text) {
  const problems = [];
  if (isPage(entry.path) && !hasNote(text)) problems.push(`${entry.path}: does not open with the kiln note (${NOTE_OPENING})`);
  if (sha256(isPage(entry.path) ? copyBody(text) : text) !== entry.kiln_sha256) problems.push(`${entry.path}: body does not match its recorded hash — changed without recording it (node scripts/upstream.mjs record)`);
...
/** A source skill named in a copy or a kiln page must be a file kiln ships under that name. A copy's note is where it cites its source, so it is not read. */
function danglingNames(root, files) {
  const shipped = new Set(files.map((file) => basename(file, ".md")));
  return files.flatMap((file) => {
    const raw = readFileSync(file, "utf8");
    const text = hasNote(raw) ? copyBody(raw) : raw;
