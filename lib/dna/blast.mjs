...
function reached(store, hits) {
  ...
  // A finding's owner may be an excluded entry, which serves no business ability.
  ...
  for (const finding of live) {
    ...
    if (count > 0) credit(score, { finding, count, feature: features.has(finding.feature_id) ? finding.feature_id : null });
  }
