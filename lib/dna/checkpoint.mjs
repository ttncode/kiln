 ...
 * under `refs/worktree/`, one ref per project path, because two kiln projects in one repository,
...
function location(root) {
  ...
  return { prefix: [...path, ".kiln", "dna", "store"], ref: `refs/worktree/kiln/dna-checkpoint/${path.length > 0 ? path.map((part) => part.replace(/[^\w.-]/g, "_")).join("/") : "_root"}` };
}
...
export function checkpoint(root) {
  try {
    const { prefix, ref } = location(root);
    const parent = tipOf(root, ref);
    const tree = storeTree(root, prefix);
    ...
    git(root, { args: [...NO_HOOKS, "update-ref", ref, commit, parent ?? ""] });
    return { commit };
  } catch (error) {
...
export function checkpointInfo(root) {
  try {
    const tip = tipOf(root, location(root).ref);
...
export function restoreCheckpoint(root) {
  ...
  const { prefix, ref } = location(root);
  const tip = tipOf(root, ref) ?? (() => { throw new DnaError("there is no checkpoint of this project's store to restore from."); })();
