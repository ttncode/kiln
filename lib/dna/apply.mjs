...
import { blobsAt, repositories } from "./scan.mjs";
...
/**
 * A scan round's files enter the ledger at the blob their commit holds. The pin moves only when
 * that commit is still the integration branch's tip (D80): a round read at any other commit is
 * recorded, but it does not get to say what the whole store describes.
 */
function recordScan(root, { scan, config, scanned }) {
  ...
  const read = current.map((repo) => ({ ...repo, commit: scan.commits?.[repo.name] ?? null }));
  const ledger = { ...scanned };
  for (const repo of read) {
    const files = (scan.files ?? []).filter((path) => repoOwning(read, path) === repo);
    if (files.length === 0) continue;
    if (!repo.commit) throw new DnaError(`the scan names files in ${repo.name} but no commit for it under scan.commits.`);
    Object.assign(ledger, Object.fromEntries(blobsAt(repo, files)));
  }
