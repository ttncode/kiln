...
import { carryCitations, movedPins } from "./carry.mjs";
...
import { missingPins, storeCitationProblems } from "./recite.mjs";
...
const findingIdentity = (finding) => JSON.stringify([finding.module, finding.proposition, finding.evidence ?? null]);

/**
 * D181: a new finding has no id, so a batch applied twice records every one of them twice —
 * which two sessions finishing one round did to 16 findings of a real store. tps-project-dna's
 * apply_intake stops at an id it already holds ("already applied? — not inserting twice"); with
 * no id yet, a finding is known by what it says and where. Refused rather than skipped, since
 * the batch's `@key`s would otherwise name records it did not write.
 */
function refuseRepeatedFindings(drafts, findings) {
  const recorded = new Map(findings.map((row) => [findingIdentity(row), row.id]));
  for (const draft of drafts.filter((each) => !each.id)) {
    const identity = findingIdentity(draft);
    if (recorded.has(identity)) throw new DnaError(`${recorded.get(identity)} already records "${draft.proposition}" with the same module and evidence; was this batch applied already? Nothing was written. A finding is changed by upserting its id.`);
    recorded.set(identity, "this batch");
  }
}
...
const sameRound = (round, { started, files }) => round.started === started && JSON.stringify(round.files) === JSON.stringify(files);

/**
 * D177: one skeleton's timing, kept beside the ledger the way dbt keeps each node's execution
 * time. `started` is stamped by `kiln dna scan --out` or `update --out`, `finished` is now; a start
 * in the future or more than a day back measures a pause rather than a round, and a skeleton
 * applied again is the same work, so neither is recorded.
 */
function roundOf(batch, { now, rounds }) {
  const started = batch.scan?.started;
  if (!Number.isFinite(started) || started > now || now - started > DAY_SECONDS) return [];
  const files = [...batch.scan.files].sort();
  if (rounds.some((round) => sameRound(round, { started, files }))) return [];
  const read = new Map((batch.read ?? []).map((entry) => [entry.path, entry.lines]));
  const lines = files.reduce((sum, path) => sum + (Number.isFinite(read.get(path)) ? read.get(path) : 0), 0);
  return [{ started, finished: now, files, lines }];
}
...
/**
 * D182: the store's records move with a pin before the batch's own upserts land, since those
 * cite the new commit already — and one the batch re-cites is not left for a reader.
 */
function carryOldCitations(root, { data, batch, config, pins }) {
  const carried = carryCitations(data, movedPins(root, { config, ...pins }));
  const recited = new Set(Object.values(batch.upsert ?? {}).flat().filter((draft) => draft.id && draft.evidence).map((draft) => draft.id));
  return { data: carried.data, report: { ...carried.report, touched: carried.report.touched.filter((id) => !recited.has(id)) } };
}
...
export function planBatch(root, { batch, today, config }) {
  ...
  checkEvidence(root, { batch, settings, config, pins: current.manifest?.source_pins });
  refuseRepeatedFindings(batch.upsert?.findings ?? [], current.data.findings);
  ...
  const carried = carryOldCitations(root, { data: stripped, batch, config, pins: { from: current.manifest?.source_pins ?? {}, to: pins } });
  const base = applyRemovals(carried.data, batch.remove ?? {});
  ...
  const rounds = [...current.rounds, ...roundOf(batch, { now: Math.floor(Date.now() / 1000), rounds: current.rounds })];
  return { data, settings, gates, scanned, rounds, pins, carried: carried.report, assigned: state.assigned, previous: current.manifest };
}
...
export function applyBatch(root, { batch, today, config }) {
  ...
  writeStore(root, { data: plan.data, settings: plan.settings, scanned: plan.scanned, rounds: plan.rounds, manifest });
 ...
 */
export function checkStore(root, config) {
  const store = readStore(root);
  const { data, settings, manifest } = store;
  ...
  const cited = citationWarning(root, { store, config });
  const results = [...gates.results, derivation, ...cited];
  return { results, failed: results.filter((result) => result.bad?.length > 0), warnings: [...gates.warnings, ...cited], manifest };
}

const EXAMPLES = 3;

const LABEL = "citations resolve at the store's pinned commits";

function citationRows(root, { store, config }) {
  const missing = missingPins(root, { store, config });
  const problems = storeCitationProblems(root, { store, config });
  const sample = problems.slice(0, EXAMPLES).map((each) => `${each.record}: ${each.problem}`).join("; ");
  return [
    ...(missing.length > 0 ? [{ label: "pinned commits are in this clone", warn: `${missing.join(", ")} missing — run git fetch, or unshallow the clone; citations there were judged at the tip` }] : []),
    ...(problems.length > 0 ? [{ label: LABEL, warn: `${problems.length} citation(s) in ${new Set(problems.map((each) => each.record)).size} record(s) do not — \`kiln dna recite\` lists them and re-cites the mechanical ones, counted in records. ${sample}` }] : []),
  ];
}

/**
 * D178: old citations are reported, not blocked — a round must not stall on debt it did not
 * write — and a checkout that cannot be read costs the check its judgement, never the status.
 */
function citationWarning(root, { store, config }) {
  if (!config) return [];
  try {
    return citationRows(root, { store, config });
  } catch (error) {
    if (!(error instanceof DnaError)) throw error;
    return [{ label: LABEL, warn: `not judged — ${error.message}` }];
  }
}
...
export function remapPlan(root, { plan, apply }) {
  ...
  writeStore(root, { data, settings: current.settings, scanned: current.scanned, rounds: current.rounds, manifest: buildManifest(root, { data, settings: current.settings, previous: current.manifest }) });
