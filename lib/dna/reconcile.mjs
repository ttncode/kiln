 ...
 */
export function reconcilePanels(panels, { minAgree, roster: given }) {
  // The source treats a roster that parsed to nothing as no roster at all, and says so.
  const roster = given?.length > 0 ? given : null;
  ...
  return {
    ...
    missing_from_every_panel: roster ? [...new Set(roster)].filter((id) => !placed.has(id)).sort(compareIds) : [],
