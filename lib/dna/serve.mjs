...
function vendored(name) {
  if (name === "" || name === "explorer.html") return PAGE;
...
function answer(root, { request, token, port }) {
  ...
  if (!route || !existsSync(route.file)) return { status: 404, body: "not found" };
  return { status: 200, type: route.type, body: request.method === "HEAD" ? undefined : readFileSync(route.file) };
}
