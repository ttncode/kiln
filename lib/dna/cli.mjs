...
export const DNA_USAGE = `  kiln dna [status]
  ...
  kiln dna serve [--port <n>]
      ...
      token no other page can guess. Read-only; stops after 30 minutes unused.
...
function printDrift(out, { drift, store }) {
  ...
  out(`  ${drift.changed.length} changed · ${drift.added.length} new candidate(s) · ${drift.deleted.length} deleted — ${VERDICTS[verdict.level]}`);
...
async function runServe(root, { argv, out }) {
  ...
  const { url, server } = await serveExplorer(root, { port });
  out(`DNA explorer: ${url}\nRead-only, on this machine only; it stops after 30 minutes unused, or on Ctrl+C.`);
