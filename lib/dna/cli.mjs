...
import { recitePlan } from "./recite.mjs";
...
import { draftInfra, undeclaredComponents } from "./infra.mjs";
...
import { groupFiles, isExcluded, scanProject, skeletons } from "./scan.mjs";
...
export const DNA_USAGE = `  kiln dna [status]
  ...
  kiln dna recite [--apply]
      Citations already in the store that do not resolve at its pinned commits. The
      mechanical ones — a repository's directory as src, a ref carrying it, L12-30 —
      are re-cited through the gated write with --apply; the rest are listed for a
      batch written by hand.
  ...
  kiln dna restore
      Put the store back from this project's last checkpoint (under refs/kiln/) when the
...
function runStatus(root, { out, config }) {
  if (!hasStore(root)) return noStore(root, out);
  const report = checkStore(root, config);
...
function runCheck(root, { out, config }) {
...
function runApply(root, { argv, out, config }) {
  ...
  if (plan.carried.moved > 0 || plan.carried.touched.length > 0) out(carriedLine(plan.carried));
  out(checkpointLine(plan.checkpoint));
  return 0;
}

function carriedLine({ moved, touched }) {
  const rewritten = touched.length > 0 ? `; ${touched.length} cite lines this change rewrote and still read as before — re-cite them by id: ${touched.join(", ")}` : "";
  return `The pin moved: ${moved} record(s) had their citations carried to the new lines${rewritten}.`;
}
...
function excludedLine(files) {
  const count = files.filter(isExcluded).length;
  return `left out as vendored or generated: ${count} — mark more with linguist-vendored in .gitattributes or .git/info/attributes (D170)`;
}
...
const history = (store) => ({ updates: store.data.updates, rounds: store.rounds });
...
function runScan(root, { argv, out, config }) {
  ...
  out(`  ${excludedLine(files)}`);
  const unscanned = files.filter((file) => file.class === "CANDIDATE" && !file.scanned);
  for (const line of sizeLines(unscanned, history(store))) out(`  ${line}`);
  const next = unscanned.slice(0, limit);
  if (next.length < unscanned.length) out(`  this run takes ${next.length} of them (--limit ${limit}): ${sizeLines(next, history(store))[0]}`);
...
function runInfra(root, { argv, out, config }) {
  ...
  const undeclared = undeclaredComponents(root);
  if (undeclared.length > 0) out(`Not in components.custom, so read as VENDOR: ${undeclared.join(", ")} — add the ones the project owns (D176).`);
  return 0;
}
...
function runUpdate(root, { argv, out, config }) {
  ...
  for (const line of sizeLines([...drift.changed, ...drift.added], history(store))) out(`  ${line}`);
  const dir = flagValue(argv, "--out");
  if (!dir) return 0;
  const target = scratchTarget(root, dir);
  mkdirSync(target, { recursive: true });
...
function runRecite(root, { argv, out, config }) {
  if (!hasStore(root)) throw new DnaError("No DNA store in this project yet (.kiln/dna/store/).");
  const plan = recitePlan(root, { store: readStore(root), config });
  for (const each of plan.leftover) out(`  needs a reader — ${each.record}: ${each.problem}`);
  if (plan.fixed.length === 0) return out(`No citation has a mechanical fix; ${plan.leftover.length} need re-citing by hand, through a batch.`) ?? 0;
  if (!argv.includes("--apply")) return out(`${plan.fixed.length} record(s) would be re-cited: ${plan.fixed.join(", ")}. Nothing written; run again with --apply.`) ?? 0;
  const applied = applyBatch(root, { batch: plan.batch, today: today(), config });
  out(`Re-cited ${plan.fixed.length} record(s) through the gated write.`);
  out(checkpointLine(applied.checkpoint));
  return 0;
}
