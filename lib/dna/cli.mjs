...
export const DNA_USAGE = `  kiln dna [status]
  ...
  kiln dna restore
      Put the store back from its last checkpoint (refs/kiln/dna-checkpoint) when the
...
function runApply(root, { argv, out, config }) {
  ...
  out(plan.checkpoint ? `Checkpoint: ${plan.checkpoint.slice(0, 12)} on refs/kiln/dna-checkpoint.` : "No checkpoint: this is not a git repository, or git refused. The store on disk is written.");
  return 0;
}
...
function runReconcile(root, { argv, out }) {
  ...
  const minAgree = Number(flagValue(argv, "--min-agree") ?? Math.floor(panels.length / 2) + 1);
...
function runRemap(root, { argv, out }) {
  const result = remapPlan(root, { plan: readBatch(argv.find((arg) => !arg.startsWith("--"))), apply: argv.includes("--apply") });
  ...
  if (result.checkpoint) out(`Checkpoint: ${result.checkpoint.slice(0, 12)} on refs/kiln/dna-checkpoint.`);
  return 0;
}
