...
function references({ data }) {
  ...
  const ancestors = [
    ...data.features.flatMap((row) => ["capability_id", "domain_id"].filter((key) => row[key] && !known.has(row[key])).map((key) => `${row.id}.${key}`)),
    ...data.processes.flatMap((row) => ["flow_id", "stage_id"].filter((key) => row[key] && !known.has(row[key])).map((key) => `${row.id}.${key}`)),
  ];
...
const GATES = [conservation, identity, references, descriptions, plannedFeatures, plannedSurfaces, plannedFindings, debts, infra, edgeKinds, actors, ruleBuckets, components, reachability, stamp];
