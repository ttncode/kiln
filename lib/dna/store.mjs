...
export function readStore(root) {
  ...
  return { data, manifest, settings: readJson(join(dir, "settings.json")) ?? {}, scanned: readJson(join(dir, "scan.json"))?.files ?? {} };
}
...
function writeFiles(dir, { data, settings, manifest, scanned }) {
  ...
  writeFileSync(join(dir, "scan.json"), `${JSON.stringify({ files: sorted }, null, 1)}\n`, "utf8");
