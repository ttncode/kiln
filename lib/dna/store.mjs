...
/**
 * `.kiln/dna/store/` holds the contract's JSONL files and `manifest.json`, plus kiln's
 * `settings.json` — what the source kept beside the store in `dna-store.config.json`,
 * `_infra_map.json`'s `components.custom` and `_actor_registry.json`. Keeping it inside means
 * one directory swap replaces both, so a store is never derived from settings it does not hold.
 */
...
export function readStore(root) {
  ...
  return { data, manifest, settings: readJson(join(dir, "settings.json")) ?? {} };
}

export function buildManifest(root, { data, settings, previous }) {
  return {
...
function writeFiles(dir, { data, settings, manifest }) {
