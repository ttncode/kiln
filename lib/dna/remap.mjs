...
function deleteAndRestate(tree, plan) {
  for (const doomed of plan.deletes ?? []) {
    ...
    if (found.capability.features.length > 0) fail(`${doomed.capability} is not empty (${found.capability.features.length} features): a non-empty node is a move, not a delete`);
    found.domain.capabilities = found.domain.capabilities.filter((each) => each !== found.capability);
  }
  restate(tree, plan);
}
...
function restate(tree, plan) {
  for (const restated of plan.restatements ?? []) {
    ...
    for (const [key, value] of Object.entries(restated).filter(([key]) => key !== "domain")) {
      if (domain[key] !== undefined && domain[key] !== value && domain[`legacy_${key}`] === undefined) domain[`legacy_${key}`] = domain[key];
      domain[key] = value;
    }
  }
}
...
function rewriteRow(row, { name, map }) {
  ...
  return { ...rewrite(row, { map, prose: CAPABILITY_AXIS.has(name) }), ...kept };
}
