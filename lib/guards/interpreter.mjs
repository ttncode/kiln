...
/**
 * The whole command is searched, heredoc bodies included: `python3 - <<EOF` carries its
 * program in the body, which is exactly the part every other scan here skips as data.
 */
export function interpreterReach(command) {
  const text = String(command ?? "");
  const part = segmentsOf(text).find(readsInlineProgram);
  if (!part) return null;
  const named = ENFORCEMENT.find((file) => text.includes(file));
  return named ? { interpreter: wordsOf(part)[interpreterAt(wordsOf(part))], file: named } : null;
}
