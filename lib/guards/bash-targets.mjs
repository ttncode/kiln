...
const COPY_VERBS = new Set(["cp", "mv", "tee", "install", "rsync"]);
...
function lexChar(lexer, i) {
  ...
  if (ch === "\\") return 2;
  if (lexer.quote === null && text.startsWith("$((", i)) return inertSpan(lexer, i);
...
function unquotedChar(lexer, i) {
  ...
  else return inertSpan(lexer, i) || heredocAt(lexer, i);
  return 1;
}

/**
 * Two places a `<<` is not a heredoc: a comment, and arithmetic. Reading one there opened a
 * "body" that swallowed every line up to a chosen word — `true # <<true`, then any command,
 * then `true` — and every guard read the hidden line as data. A comment runs to the end of
 * its line; `$((…))` and `((…))` run to their closing pair. Both are consumed whole.
 */
function inertSpan(lexer, i) {
  const { text } = lexer;
  if (text[i] === "#" && (i === 0 || /\s/.test(text[i - 1]))) {
    const end = text.indexOf("\n", i);
    return (end === -1 ? text.length : end) - i;
  }
  const open = text.startsWith("$((", i) ? 3 : text.startsWith("((", i) ? 2 : 0;
  if (open === 0) return 0;
  const close = text.indexOf("))", i + open);
  return (close === -1 ? text.length : close + 2) - i;
}
...
function lexHeredocs(command) {
  ...
  while (i < text.length) {
    if (text[i] === "\n" && lexer.quote === null && lexer.pending.length > 0) {
      ...
      if (resume === null) return { text, bodies: [] };
      [bodies[bodies.length], out, lexer.pending, i] = [text.slice(i + 1, resume), `${out}\n`, [], resume];
      continue;
    }
...
export function withoutHeredocBodies(command) {
  return lexHeredocs(command).text;
}

/** The heredoc bodies, terminator lines included, for the one caller that must read them. */
export function heredocBodiesOf(command) {
  return lexHeredocs(command).bodies;
}
 ...
 *
 * Only an assignment that certainly runs is believed. One behind `||` or `&&`, inside an
 * `if` or a loop, or written anywhere but at the start of its command may not have run, and
 * believing it let `W=src; true || W=.kiln/tmp/<id>; echo x > $W/app.ts` be read as scratch
 * while the shell wrote source. Such a variable becomes unknown for the rest of the line.
 */
const OPENS_BLOCK = /^(?:if|while|until|for|case|select)\b/;
const CLOSES_BLOCK = /^(?:fi|done|esac)\b/;

function assignmentIsCertain(entry, depth) {
  return depth === 0 && (entry.operator === null || entry.operator === ";" || entry.operator === "\n");
}
...
function withLiteralAssignments(parts) {
  ...
  return parts.map((entry) => {
    ...
    if (OPENS_BLOCK.test(part)) depth += 1;
    const assigned = ASSIGNMENT.exec(part);
    if (assigned && assignmentIsCertain(entry, depth)) known.set(assigned[1], assigned[2].replace(/^["']|["']$/g, ""));
    else for (const name of assignedNames(part)) known.set(name, `$${name}`);
    if (CLOSES_BLOCK.test(part)) depth = Math.max(0, depth - 1);
    return { ...entry, part };
  });
}

/** Every name a segment may assign, wherever in it the assignment sits. */
function assignedNames(part) {
  return [...part.matchAll(/(?:^|[\s;({!])(?:export\s+)?([A-Za-z_]\w*)=/g)].map((match) => match[1]);
}
 ...
 */
export function argumentsOf(part) {
...
export function redirectTargets(part) {
...
/**
 * `tee` writes its operands from wherever it sits in a pipeline — `echo x | tee -a file` is the
 * common shape, and reading only a segment's first word missed it.
 */
function copyTargets(part) {
  const tokens = operands(part);
  const tee = tokens.indexOf("tee");
  if (tee !== -1) return tokens.slice(tee + 1).filter((token) => token !== "|");
  return COPY_VERBS.has(tokens[0]) ? tokens.slice(-1) : [];
}
...
/**
 * A device is where output goes to be thrown away or shown, not a file anything is written
 * into. `2>/dev/null` sits in half the commands an agent runs, and read as a write outside
 * the project it refused them all while a work was open.
 */
...
export function writeTargets(command) {
  return segmentsOf(command)
    .flatMap((part) => [...redirectTargets(part), ...copyTargets(part), ...sedTargets(part)])
    .filter((path) => !DEVICE.test(path));
}

/**
 * Verbs that take a file away from where it was, and verbs that take away the right to read
 * it. `chmod 000` on a work directory was the quietest of them: the guard could no longer read
 * the state inside and treated the work as not there.
 */
const REMOVAL_VERBS = new Set(["rm", "unlink", "shred", "truncate"]);
const PERMISSION_VERBS = new Set(["chmod", "chown", "chattr"]);

/**
 * The operands after the verb, not after the first word of the segment: in
 * `cat .kiln/config.json | grep chmod` the config is `cat`'s operand, and reading it as the
 * target of a `chmod` refused a read.
 */
function afterVerb(part, verbs) {
  const words = argumentsOf(part);
  const at = words.findIndex((word) => verbs.has(word.split("/").pop()));
  return at === -1 ? [] : words.slice(at + 1).filter((word) => !word.startsWith("-")).map(expandHome);
}

/** chmod, chown and chattr: the files whose readability or ownership the command changes. */
export function permissionTargets(command) {
  return segmentsOf(command).flatMap((part) => afterVerb(part, PERMISSION_VERBS));
}
...
export function removalTargets(command) {
  return segmentsOf(command).flatMap((part) => [...afterVerb(part, REMOVAL_VERBS), ...movedAway(part)]);
}
...
const CLEAN_TAKES_VALUE = new Set(["-e", "--exclude"]);

function cleanPaths(rest) {
  const end = rest.indexOf("--");
  const flags = end === -1 ? rest : rest.slice(0, end);
  const named = flags.filter((token, at) => !token.startsWith("-") && !CLEAN_TAKES_VALUE.has(flags[at - 1]));
  return end === -1 ? named : [...named, ...rest.slice(end + 1)];
}

/**
 * D200: a forced `git clean` removes what its paths name, from where `-C` puts it. The old test
 * was a regex wanting `f` and `x`/`d` in one cluster, and it answered `.` whatever the command
 * said, so `git -C <outside> clean -xfd` and `git clean -f -d -x <outside>` passed D34's
 * check for "outside the project". A dry run — a token that is exactly `-n` or `--dry-run`, as
 * cc-safety-net reads it — removes nothing.
 */
export function gitCleanTargets(part) {
  const invocation = gitInvocation(argumentsOf(part));
  if (!invocation || invocation.args[0] !== "clean") return null;
  const rest = invocation.args.slice(1);
  const options = rest.includes("--") ? rest.slice(0, rest.indexOf("--")) : rest;
  // An -e / --exclude value is a pattern, not a flag: `git clean -fd -e -n <dir>` cleans for real.
  const flags = options.filter((token, at) => !CLEAN_TAKES_VALUE.has(options[at - 1]));
  const forced = flags.some((token) => /^-[a-zA-Z]*f/.test(token) || (token.length >= 4 && "--force".startsWith(token)));
  const dry = flags.some((token) => token === "-n" || (token.length >= 4 && "--dry-run".startsWith(token)));
  if (!forced || dry) return null;
  const base = gitWorkdirs(invocation.words).reduce((dir, next) => (isAbsolute(next) ? next : join(dir, next)), ".");
  const paths = cleanPaths(rest);
  return (paths.length > 0 ? paths : ["."]).map((path) => expandHome(isAbsolute(path) ? path : join(base, path)));
}

export function destructiveTargets(command) {
  return segmentsOf(command).flatMap((part) => recursiveRemoval(part) ?? gitCleanTargets(part) ?? []);
}
