...
export function holdsControl(root, target) {
  ...
  if (isStoreEntry(root, target)) return true;
  return isInside(work, target) && target.slice(work.length + 1).split(sep).length === 1;
}

const STORE_ENTRY = /^store(?:\.[^/]*)?(?:\/|$)/;

/**
 * D183: what a removal may not reach under `.kiln/dna/` is the store and the copies its write
 * swaps through (`store.building`, `store.previous`). D169 guards every other sibling against a
 * write, because a file planted there is one no gate saw; taking a file away plants nothing,
 * and refusing it left a real session unable to clear a stray note it had copied in.
 */
function isStoreEntry(root, target) {
  const dna = join(kilnDir(root), "dna");
  return isInside(dna, target) && STORE_ENTRY.test(target.slice(dna.length + 1).split(sep).join("/"));
}
