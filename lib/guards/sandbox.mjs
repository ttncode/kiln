...
export function holdsControl(root, target) {
  ...
  const anchors = [join(kilnDir(root), "config.json"), work, join(kilnDir(root), "hooks"), join(root, ".git", "hooks")];
 ...
 */
const DNA_STORE = "the DNA store is written only by `kiln dna apply <batch.json>`, which assigns the ids and runs the store's gates; draft the batch under .kiln/tmp/ and apply it";
...
export function sandboxVerdict(root, { target, activeId }) {
  ...
  if (isInside(join(kilnDir(root), "dna"), target)) return { blocked: true, reason: DNA_STORE };
