...
import { segmentsWithOperators } from "./bash-targets.mjs";
...
/**
 * `sequential` answers "did the previous command's effect survive to here", which is what a
 * `cd` needs: anything but `||` carries it. `operator` is the raw join, because a branch
 * created by `checkout -b` needs the stricter question — only `&&` proves the creation
 * succeeded, while `;` runs the next command on the old branch anyway.
 *
 * The split is the lexer's, not a regex over the raw text: a heredoc body is data, and
 * reading `cd /elsewhere && git commit` out of a note being written to a file refused the
 * write as a commit on a protected branch.
 */
export function splitWithOperators(command) {
  return segmentsWithOperators(command);
}
