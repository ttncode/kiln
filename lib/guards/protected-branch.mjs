...
import { wordsOf } from "./bash-targets.mjs";
...
function branchMove(part) {
  const invocation = gitInvocation(wordsOf(part));
  if (!invocation || invocation.shell) return null;
  ...
  // `git checkout <name>` may restore a file called that rather than move to a branch, and
  // reading it as a certain move let `git checkout src/app.ts && git commit` commit to main.
  // `switch` only ever moves, so only `switch` is certain.
  return name ? { name, creates: false, certain: args[0] === "switch" } : null;
}
...
export function branchChain(command) {
  ...
  return splitWithOperators(command).map(({ part, operator }) => {
    ...
    current = move ? { ...move, certain: move.certain ?? true } : carried;
    return carried;
  });
}
...
function violationIn(part, context) {
  ...
  if (invocation.shell) return violationIn(invocation.shell, context);
  const { args } = invocation;
  const [subcommand] = args;
