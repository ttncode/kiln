 ...
 *
 * Only the **creating** forms, and only across `&&`. That is not shell interpretation
 * creeping back in; it is the same three-operator reading `cwdChain` already does for `cd`,
 * and the operator choice is what makes it sound. With `&&` a failed `checkout -b` stops the
 * chain, so the later command runs only if the branch was created. With `;` it runs anyway,
 * on the old branch — so `;` carries nothing. `||` runs it *because* the checkout failed.
 *
 * The created name is still compared against `vcs.protected` like any other: resolving to it
 * is not the same as trusting it.
 */
function createdBranch(part) {
...
export function branchChain(command) {
  ...
  return splitWithOperators(command).map(({ part, operator }) => {
    const carried = operator === "&&" ? current : null;
    current = createdBranch(part) ?? carried;
    return carried;
  });
}
...
function violationIn(part, context) {
  ...
  const repo = context.created ? { ...here, branches: [context.created], resolved: true } : here;
...
export function protectedBranchViolation({ command, protectedBranches = [], currentBranch = null, resolveRef = () => null, repoFor = null }) {
  ...
  const created = branchChain(command);
  for (let index = 0; index < parts.length; index += 1) {
    const found = violationIn(parts[index], { protectedBranches, repoFor: forPart, index, created: created[index] });
