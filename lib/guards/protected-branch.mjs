 ...
 */
function pushTargets(args, { currentBranch, resolveRef }) {
...
function targetsFor(args, context) {
  const [subcommand] = args;
  if (subcommand === "push") return pushTargets(args, context);
  if (WRITE_OPS.has(subcommand)) return [context.currentBranch];
  return [];
}
...
function violationIn(part, context) {
  ...
  const hit = targetsFor(args, context)
    .filter(Boolean)
 ...
 */
export function protectedBranchViolation({ command, protectedBranches = [], currentBranch = null, resolveRef = () => null }) {
  if (!command || !command.includes("git")) return null;
  for (const part of splitCommands(command)) {
    const found = violationIn(part, { protectedBranches, currentBranch, resolveRef });
