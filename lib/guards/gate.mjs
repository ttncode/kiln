...
import { AUTHORIZING, SHIP_AUTHORIZING, artifactPath, hashArtifact } from "./../state.mjs";
 ...
 * checked on every gate key, not only the one that authorizes source edits. A record with
 * no hash binds to nothing, so it authorizes nothing.
 */
export function gateStillBinds(root, { state, key }) {
  ...
  const path = artifactPath(root, { id: state.id, key });
  const name = path.split(sep).pop();
  if (!record.artifact_sha) return { ok: false, reason: `the ${key} gate was recorded without the ${name} it approves — record it again` };
  if (!existsSync(path)) return { ok: false, reason: `${name} is gone, but its approval is still on record` };
  if (hashArtifact(path) !== record.artifact_sha) {
    return { ok: false, reason: `${name} changed after it was approved` };
  }
  return { ok: true };
}
