...
/** A size kiln cannot read — the file vanished, or the permission is gone — is a file it skips. */
function smallEnough(path) {
  try {
...
/**
 * D197: a symbolic link is not followed, as ripgrep and `git grep` do not by default. A loop
 * or a dangling link used to stop the whole search; a link into the tree loses nothing, since
 * the file it names is found at its own path. Nothing the walk cannot read stops it either: a
 * directory it may not list and a file it may not stat are skipped, as ripgrep skips them.
 */
/** A directory kiln may not list — a container's volume owned by root, say — holds nothing to search. */
...
function walk(dir, root) {
  return entriesOf(dir).flatMap((entry) => {
    if (SKIP.has(entry.name) || entry.name === ".kiln" || entry.isSymbolicLink()) return [];
    const path = join(dir, entry.name);
    if (entry.isDirectory()) return walk(path, root);
    return TEXT.test(entry.name) && smallEnough(path) ? [relative(root, path)] : [];
  });
}
...
/**
 * A submodule is one gitlink in its superproject's tree — its HEAD — so an uncommitted edit
 * inside it left the tree id unchanged and a stale run read as current, on exactly the
 * monorepos kiln targets. Each checkout's own tree is part of the answer.
 */
export function checkoutTrees(root) {
  return checkoutsUnder(root).map((dir) => ({ dir: relative(root, dir) || ".", tree: checkoutTree(dir) }));
}

/** The tree a verify run describes, from the rows `checkoutTrees` measured. */
export function worktreeTree(root, trees = checkoutTrees(root)) {
  if (trees.some((row) => row.tree === null)) return null;
  return trees.length === 1 ? trees[0].tree : trees.map((row) => `${row.dir}:${row.tree}`).join(" ");
}
