...
/**
 * A word reserved for later carries what it is waiting for, and what exists instead. `dna`
 * resolving as a bare `reserved` told the orchestrator to *run* a command kiln does not
 * ship, so `/kiln dna` answered with a usage error and never said the one thing asked about.
 */
const RESERVED_MESSAGES = Object.freeze({
  dna: "DNA is Knowledge tier 1-2, and this version of kiln does not ship it yet.\n" +
...
export function resolveArgument({ arg, root, config = {}, now = new Date() }) {
  ...
  if (reserved) {
    return { kind: "reserved", command: reserved, rest: arg.trim().slice(reserved.length).trim(), message: RESERVED_MESSAGES[reserved] ?? null };
  }
