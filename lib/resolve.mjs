 ...
 */
/**
 * A leading ceremony word is an instruction to kiln, not part of what is being asked. Left
 * in, `/kiln full This route returns a 500` minted
 * `20260921-full-route-http-localhost-2380-admin` — every id from then on carrying a word
 * that describes the process rather than the work. Only a leading one is dropped: `full`
 * in the middle of a sentence is somebody's actual subject.
 */
export function withoutPathWord(text) {
  return String(text).replace(new RegExp(`^\\s*(?:${PATHS.join("|")})\\b[\\s:,—-]*`, "i"), "");
}
...
export function resolveArgument({ arg, root, config = {}, now = new Date() }) {
  ...
  return { kind: "description", text: arg, id, action: "investigate", ...mentionsHint(root, arg) };
}
