...
function checkStack(config) {
  ...
  if (steps.length === 0) {
    return [check(FAIL, {
      title: "stack",
      detail: `"steps" in .kiln/config.json is empty, so \`kiln verify\` has nothing to run and refuses rather than reporting a pass nobody earned`,
    })];
  }
  const gaps = unsatisfiedSteps({ steps }, config.stack.cmd);
...
function checkProtected(root, config) {
  const declared = integrationBranch(config);
  if (!config.vcs.protected.includes(declared)) {
    return check(FAIL, {
      title: "protected branches",
      detail: `this project ships from "${declared}", which is not in vcs.protected — a push to it would be allowed`,
    });
  }
  const remote = remoteDefaultBranch(root);
  if (remote && remote !== declared && !config.vcs.protected.includes(remote)) {
    return check(WARN, {
      title: "protected branches",
      detail: `config ships from "${declared}"; the remote's default is "${remote}", which is not protected`,
    });
  }
