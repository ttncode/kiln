 ...
 */
function reachableRunner(checkout) {
  let dir = checkout;
  while (dir) {
    if (existsSync(runnerPath(dir))) return runnerPath(dir);
 ...
 */
/**
 * The runner is half the floor, and it was written once and never again: `installFloor`
 * wrote it only when missing. So a fix to what the floor refuses reached no project that
 * already had one — the same shape as the stale hook above, one file over.
 */
function installedState(path, checkout) {
  const body = readFileSync(path, "utf8");
  if (!body.includes(MARKER)) return "foreign";
  if (body !== hookBody()) return "stale";
  const runner = reachableRunner(checkout);
  if (!runner) return "no-runner";
  return readFileSync(runner, "utf8") === runnerBody() ? "installed" : "stale";
}

function writeRunner(runner) {
  writeFileSync(runner, runnerBody(), "utf8");
  return runner;
}

/** Writes only what is missing or kiln's own and out of date — the law `kiln init` follows. */
export function installFloor(root) {
  ...
  else if (readFileSync(runner, "utf8") !== runnerBody()) written.push(writeRunner(runner));
