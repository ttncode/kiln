...
exec node "$(dirname "$0")/../../.kiln/hooks/pre-push.mjs" "$@"
`;
}
...
function hookPath(checkout) {
  const dir = gitOutput(checkout, ["rev-parse", "--absolute-git-dir"]);
  return dir ? join(dir, "hooks", "pre-push") : null;
}
...
export function floorStatus(root) {
  return checkoutsUnder(root).map((checkout) => {
    const path = hookPath(checkout);
    if (!path) return { checkout, state: "not-a-repo" };
    if (gitOutput(checkout, ["config", "--get", "core.hooksPath"])) return { checkout, path, state: "shadowed" };
