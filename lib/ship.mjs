...
import { refOfId, slugOfId } from "./resolve.mjs";
...
export function shipPlan(root, { config, state }) {
  ...
  const record = config.work?.committed === false ? null : `.kiln/work/${state.id}`;
  return { topic: state.id, branch: branchName(config, state), modules: modules.sort(sortByName), record };
}

/**
 * The path list SHIP stages, per repository — D67c's law made mechanical rather than left to
 * the agent's memory of it. It includes the work's own record where `work.committed` asks for
 * it (D36): on a real run the agent staged the two source files and left `brief.md`,
 * `plan.md` and `review.md` out, so `git log -- .kiln/work/<id>/`, which D18 names as the
 * whole history story, had nothing in it.
 */
function stageLine(row, record) {
  const inside = row.path === "." ? row.files : row.files.map((file) => file.slice(row.path.length + 1));
  const paths = row.path === "." && record ? [...inside, record] : inside;
  const where = row.path === "." ? "" : `-C ${row.path} `;
  return `  stage: git ${where}add -- ${paths.join(" ")}`;
}
...
export function branchName(config, state) {
  const values = { id: state.id, ref: refOfId(state.id), slug: slugOfId(state.id), type: state.type ?? null };
  const unfilled = [];
...
/**
 * Git's own rule for a branch name, so a pattern git would refuse is named here and not at
 * `git switch`. `--branch` also refuses what a ref name allows and a branch does not: `-foo`, `HEAD`.
 */
...
function branchLines(branch, pattern) {
  ...
  return [
    ...
    ...(missing.length === 0 && !isBranchName(rendered) ? [`  git refuses "${rendered}" as a branch name (git check-ref-format) — change vcs.branch_pattern.`] : []),
...
export function renderShipPlan(plan, pattern = "") {
  return [
    ...
    ...plan.modules.flatMap((row) => [`  ${row.path}  →  ${row.integration}   ${row.files.length} file(s)`, stageLine(row, plan.record)]),
    "",
