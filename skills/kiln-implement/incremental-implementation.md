> **In kiln:** agent-skills' skills/incremental-implementation/SKILL.md (D190), edited where it commits: kiln commits the reviewed change once, at SHIP (D142), so each slice ends recorded green by `kiln verify --task` instead of committed, and a noticed-but-not-touched item goes to the ledger as a follow-up rather than a question mid-run. A skill it names is the file of that name in kiln's skills.
...
4. **Record** -- `kiln verify <id> --phase fast --task <n>/<total>` records the slice green; kiln commits the reviewed change once, at SHIP
...
→ Recorded in the ledger; each can become a follow-up work (`--follows`) after this one ships
```
...
- [ ] The slice is recorded green with `kiln verify --phase fast --task`
...
| "These changes are too small to verify separately" | Small increments are free. Large ones hide bugs and make the failing change hard to find. |
...
- Large unverified changes accumulating
...
- [ ] Each increment was individually tested and recorded
