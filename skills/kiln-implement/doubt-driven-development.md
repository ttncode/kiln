> **In kiln:** agent-skills' skills/doubt-driven-development/SKILL.md, verbatim (D191). IMPLEMENT runs without pausing (kiln-implement's continuous execution), which makes it a non-interactive context in this file's own terms: cross-model escalation is skipped, and the skip is announced. The `agents/` roster it names is kiln-review's `personas/`.
