> **In kiln:** agent-skills' skills/source-driven-development/SKILL.md, verbatim (D191). Where it says to ask the user — a version that is missing or ambiguous — that is a blocking unknown in kiln: `kiln halt <id> --kind blocking_unknown --reason "<the question>"`. Its sdd-cache hook is not part of kiln (bash, and network access from kiln's own code).
