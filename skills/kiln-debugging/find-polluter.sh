...
# Usage: ./find-polluter.sh <file_or_dir_to_check> <test_pattern> <command that runs one test file>
# Example: ./find-polluter.sh '.git' 'src/**/*.test.ts' 'npx vitest run'

set -e

if [ $# -ne 3 ]; then
  echo "Usage: $0 <file_to_check> <test_pattern> <command that runs one test file>"
