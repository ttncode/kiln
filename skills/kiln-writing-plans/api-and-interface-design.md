> **In kiln:** agent-skills' skills/api-and-interface-design/SKILL.md, verbatim (D190). `kiln practices --stage plan` routes it when the change needs it. A skill it names is the file of that name in kiln's skills; a path it gives (references/…) is the file of that name there too.
