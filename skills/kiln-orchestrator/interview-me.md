> **In kiln:** agent-skills' skills/interview-me/SKILL.md (D192), edited in two places: the confirmed intent goes into the brief instead of `docs/intent/`, and the "offer downstream paths" step is gone because kiln's path is already chosen. Its explicit-yes gate is the one D21 adopted. Never under `--auto`: it is a dialogue. A skill it names is the file of that name in kiln's skills.
...
2. **Write it into the brief** (`.kiln/work/<id>/brief.md`, under "What is asked"). kiln's path is already classified, so there is no path to choose here.
...
In kiln the confirmed intent persists in the work's brief, which every later stage reads.
