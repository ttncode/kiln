...
| `reserved` | a command: run `kiln <command>` with `rest` as its arguments, and stop. The exception is `dna init` and `dna update`, which are not one command but the bootstrap and update playbooks: invoke the `kiln-dna` skill and follow it. A sentence that merely opens with one of these words is refused here rather than minted as work — say so, and offer to rephrase it |
