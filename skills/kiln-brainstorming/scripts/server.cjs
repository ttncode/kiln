...
const KILN_VERSION = readKilnVersion();
...
function readKilnVersion() {
  ...
  for (const manifest of manifests) {
    try {
      ...
      // A packaged plugin may omit package.json; try the next manifest.
    }
  }
...
// No image is fetched here, and none should be: the companion opening a page must
// not reach the network (D7 item 6), and MIT grants copyright rights, not trademark,
// so the upstream name and mark are not kiln's to display.
function brandMarkup() {
  const text = 'kiln v' + escapeHtmlText(KILN_VERSION);
  return '<div class="brand"><a href="https://github.com/ttncode/kiln"><span class="brand-copy">' + text + '</span></a></div>';
}
