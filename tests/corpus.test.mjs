...
const BRANCHES = [
  ...
  ["bash", "sudo git push origin main", "deny", PROTECTED],
  ["bash", "env git push origin main", "deny", PROTECTED],
  ["bash", "FOO=1 git push origin main", "deny", PROTECTED],
  ["bash", "/usr/bin/git push origin main", "deny", PROTECTED],
  ["bash", "\\git push origin main", "deny", PROTECTED],
  ["bash", "(git push origin main)", "deny", PROTECTED],
  ["bash", "command git push origin main", "deny", PROTECTED],
  ["bash", "time git push origin main", "deny", PROTECTED],
  ["bash", "nohup git push origin main", "deny", PROTECTED],
  ["bash", "git -c alias.p=push p origin main", "deny", PROTECTED],
...
const DISARMING = [
  ...
  ["bash", "git commit -u -n -m x", "deny", DISARM],
  ["bash", "git commit -S -n -m x", "deny", DISARM],
  ["bash", "git --git-dir .git commit -n -m x", "deny", DISARM],
...
const BEFORE_THE_GATE = [
  ...
  ["bash", "echo x | tee -a src/app.js", "deny", GATE],
  ...
  ["bash", "W=src; true || W=.kiln/tmp/{id}; echo pwned > $W/app.js", "deny", GATE],
  ["bash", "W=src; false && W=.kiln/tmp/{id}; echo pwned > $W/app.js", "deny", GATE],
  ["bash", "W=src; if false; then W=.kiln/tmp/{id}; fi; echo pwned > $W/app.js", "deny", GATE],
  ["bash", "echo $((1<<true))\necho pwned > src/app.js\ntrue", "deny", GATE],
...
const CONTROL_FILES = [
  ...
  ["bash", "rm -rf .kiln/work/{id}/*", "deny", CONTROL],
  ["bash", "rm .kiln/work/{id}/state.jso?", "deny", CONTROL],
  ["bash", "node -e \"require('fs').rmSync('.kiln/work',{recursive:true})\"", "deny", CONTROL],
  ["bash", "rm -rf src/*.tmp", "allow"],
  ...
  ["bash", "python3 -W ignore -c \"open('.kiln/config.json','w')\"", "deny", CONTROL],
  ["bash", "python3 -X utf8 -c \"open('.kiln/config.json','w')\"", "deny", CONTROL],
  ["bash", "timeout 5 python3 -c \"open('.kiln/config.json','w')\"", "deny", CONTROL],
  ["bash", "(python3 -c \"open('.kiln/config.json','w')\")", "deny", CONTROL],
  ["bash", "{ python3 -c \"open('.kiln/config.json','w')\"; }", "deny", CONTROL],
  ["bash", "if python3 -c \"open('.kiln/config.json','w')\"; then :; fi", "deny", CONTROL],
  ["bash", "echo $(python3 -c \"open('.kiln/config.json','w')\")", "deny", CONTROL],
  ["bash", "! python3 -c \"open('.kiln/config.json','w')\"", "deny", CONTROL],
  ["bash", "sudo -u me python3 -c \"open('.kiln/config.json','w')\"", "deny", CONTROL],
  ["bash", "nice -n 5 python3 -c \"open('.kiln/config.json','w')\"", "deny", CONTROL],
  ["bash", "uv run python -c \"open('.kiln/config.json','w')\"", "deny", CONTROL],
  ["bash", "bash -o errexit -c \"echo {} > .kiln/config.json\"", "deny", CONTROL],
  ["bash", "true # <<true\necho {} > .kiln/config.json\ntrue", "deny", CONTROL],
  ["bash", "true # <<true\nrm -rf .kiln/work\ntrue", "deny", CONTROL],
  ["bash", "cat .kiln/config.json", "allow"],
  ["bash", "cat .kiln/config.json | grep chmod", "allow"],
  ["bash", "chmod -R u+w .", "allow"],
  ["bash", "chmod +x scripts/build.sh", "allow"],
