...
| **D156** | **The infra map is drafted mechanically, and a stack file declares what counts as a surface** | The playbook's Phase 2b runs `bootstrap_infra_map.py` for a zero-token draft of services, surfaces and edges that a person reviews before anything consumes it. `kiln dna infra` keeps that shape — its output is a **batch for review**, never applied by the command, because naming a service is a decision that outlives the run — and changes where the guesses come from. **Services:** a module (D81) is a service; a single repository is one service whose roots are its code directories, split in two only where a directory is named like a front end; and a compose file's `image:` lines add the DATABASE / CACHE / QUEUE / SEARCH / GATEWAY services a project runs beside its code. Measured on kiln's own tree, the source's first-segment clustering would have drafted six BACKEND services (`bin`, `hooks`, `lib`, …) for one CLI. **Surfaces:** the source minted SCREEN from directory names and its own field audit found 17 of 25 were widget libraries; kiln mints a surface only where a rule the stack file declares (`stacks/<id>.json` → `dna.surfaces`: a path pattern and, optionally, a content pattern) matches a file at the integration branch — Next.js pages and API routes, HTTP route registrations and cron schedules for `node`; view, JSON and CLI controllers and models for `php-ci3`. A new stack teaches the draft by adding rules, not code, which is D61's extension point doing the job D107 said `bootstrap_dd_sources.py`'s 2,495 stack-specific lines were doing. Edge hints from finding prose are left out: the source measured its first version at 0 useful of 8, and hop-lift (D153) derives topology from cited call sites instead. | 09-23 |
