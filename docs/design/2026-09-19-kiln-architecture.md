...
> Section 1 (v1 scope) and Sections A–H all locked. The decision log runs to **D177**. It reached D89 after five
...
> **D86–D124** from building it and running it on real projects, and **D125–D151** from the
> pre-v1.0 audit that read this file against the product and against mature open-source
> projects, and **D152–D169** from building the Knowledge tier (DNA) on branch `feat/dna`, and **D170–D177** from its first real bootstrap. Tier D is graded (`2026-09-21-acceptance-c6-handover.md`). What remains for a v1.0
...
| **D150** | **A literal assignment earlier in the line is expanded where it is used; an interpreter reads stdin only when something is sent there (refines D129)** | Both found as false blocks in the acceptance run's auto session, each on the investigation itself. `W=.kiln/work/<id> && cat > $W/brief.md` read `$W/brief.md` as a literal path in the project — source, before the gate. And `node --version` beside a `cat .kiln/config.json` was refused: with no script named it was taken for a program on stdin, and the file name was searched in the whole line rather than in the program. cc-safety-net resolves paths from literal assignments and nothing else; kiln adopts that rule — a standalone `NAME=value` or `export NAME=value` whose value holds no `$` or backtick is substituted into the commands after it, and any other variable stays unread. An interpreter with no program reads stdin only behind a pipe, a heredoc or a `<`, and the name is searched in that segment, plus its heredoc body when it reads one. The rerun of the same session then met a third: `cat plan.md 2>/dev/null` refused as a write outside the project, which every command silencing its errors would have met while a work was open. A device — `/dev/null`, `/dev/stdout`, `/dev/stderr`, `/dev/tty`, `/dev/fd/<n>` — is not a file anything is written into. | 09-23 |
| **D151** | **An independent review of D125–D150 found six bypasses they introduced; each is closed the conservative way, and the corpus now holds every one** | A reviewer who did not write the branch read its diff and reproduced each finding against the base, which blocked them all. **(1)** D129's interpreter reading stopped at the first non-flag word and required a simple command position, so `python3 -W ignore -c`, `timeout 5 python3 -c`, `(python3 -c …)`, `if python3 -c`, `$(python3 -c …)`, `uv run python -c` and `bash -o errexit -c` all wrote the config: the inline flag is now looked for anywhere after an interpreter word anywhere in the segment, and only the stdin case keeps command position. **(2)** D126's lexer opened a heredoc on a `<<` inside a comment or `$((…))`, so `true # <<true` hid the next line from every guard: comments and arithmetic are inert spans. A heredoc or `-c` program **fed to a shell** is a command line of its own and now goes through the whole chain, bounded in depth because a timed-out hook allows. **(3)** D150 believed every literal assignment; one behind `||`/`&&` or inside `if` may not run, and believing it read source as scratch — such a variable becomes unknown. **(4)** D131 read `git checkout <path>` as a move to a branch named after the path; a non-creating `checkout` is now only a candidate, `switch` stays certain. **(5)** D148 found projects from a fixed set of spellings; every absolute path the text names is now asked too. **(6)** `-S` and `-u` take their value only attached, so `commit -u -n` skipped the hook. Four smaller gaps closed with them: the full path now becomes `reviewed` at its review gate (amends D117 — D137's window was bounded-only), the tree fingerprint includes each submodule's own tree, the post-run audit counts a write that was reset back, and `chmod -R u+w .` is no longer refused as if the project root were a control file, while `rm` of a glob is judged by the directory it expands in. Pre-existing gaps the same pass surfaced — `tee` after a pipe, `git --git-dir .git commit -n`, `node -e` naming `.kiln/work` — are closed too. | 09-23 |
| **D152** | **INVESTIGATE asks the knowledge tier, and what it named is measured against the change** | Step 0 of the DNA plan the owner confirmed on 2026-09-23. D47 made the Knowledge port's survival depend on one observable — whether its output leads to action — and the orchestrator never called it at INVESTIGATE, where D23 puts `Knowledge.blastRadius`; tier 0 was reached only by `kiln scope` after the plan existed. `kiln blast --for <id>` now records the terms and the files it named, the orchestrator calls it before reading code, and `kiln scope` and `kiln report` print how many of the change's files it had named. That line is the bar DNA must clear on the same works: a richer tier that does not name more of what actually changed has not earned its cost. | 09-23 |
| **D153** | **The DNA store is the write model, and `kiln dna apply` is its one door** | tps-project-dna's own roadmap names the step it never took: *"v1 = DERIVED … v2 = write-model — the pipeline reads/writes the store directly"* (`dna-store.md`). kiln takes it, because kiln has no per-project working JSONs for a v1 adapter to derive from — D107 builds DNA from the source, not from another tool's output. `.kiln/dna/store/` holds the contract's sixteen JSONL files and `manifest.json`, plus `settings.json` for what the source kept beside the store (`dna-store.config.json`, `_infra_map.json`'s custom components, `_actor_registry.json`), so one directory swap replaces both and a store is never derived from settings it does not hold. **The write path.** An agent drafts a batch under `.kiln/tmp/` — records to add or change, records to remove, `key` / `@key` so findings and the feature that owns them arrive together — and `kiln dna apply` counts the ids the contract makes mechanical (`RD-`/`PRD-`, capability and feature under their parent, `BF-` flow/stage/process, dated ledger rounds), refuses to invent the ones a person chooses (`DOM-{ABBR}`, `SVC-`/`SRF-` mnemonics), strips every derived field, derives them again, runs **every gate**, writes beside the store and swaps only if the recount of the written files matches. **Three choices.** (1) Derived fields are stripped and re-derived on every write, so a store built one batch at a time is the store a clean rebuild gives — the property an incremental index has to hold (tested). (2) The source ran its gates twice, in the builder and in `check_gates.py`, and the copies had drifted (only the builder required a feature to name its capability); kiln keeps one list, run before every write and by `kiln dna check`, which adds the one gate a write path cannot run on itself: that the derived fields on disk are what the records derive to. (3) A finding is never removed (id-schemes: *"NEVER reused or renumbered"*), and an id a record names in its provenance (`merged_from`, `legacy_*`, `id_history`) is never counted free again. kiln adds one gate the source lacks — ids follow the id schemes — because a store kiln builds from nothing has no legacy ids to excuse. **Parity, measured:** the source's own `check_gates.py` passes on a store kiln wrote, and the four derivations (`service_id`, `surface_id`, components, hop-lift) produce byte-identical output to the Python originals imported and run on the same records, over 300 generated stores (4,185 findings; every derivation exercised). **The guard.** An `Edit`, `Write` or shell redirect into `.kiln/dna/` is refused with the door named. That is not a D7 line — an inline program can still write there, and `kiln dna check` finds the result — it is the one place a refusal can say where the write belongs. `kiln dna` is now a command, so the reservation message B12 required is retired: every reserved word resolves to a command the orchestrator runs. **Review.** A reviewer who had not written it found two high defects and eleven smaller, each reproduced: changing a dated ledger record rewrote its date to today, so its id and date disagreed; and a batch could author a derived field, which the write gates passed and `kiln dna check` then failed — the incremental-equals-rebuild property broken from the write side. Both are closed (a batch that sets a derived field is refused and told the field is computed), as are `id_history` read under `ext`, an explicit new id that is another record's history, the source's provenance gate (missing, so `check_gates.py` would have failed a kiln store), capability and stage parents (a `remove` could orphan them), an interrupted swap (now recovered from `store.previous` instead of read as empty), `@` in a name, `null` on an `ext` field, malformed batches (refused in words, not as a crash) and the first excluded claimant winning as in the source. The parity harness is `scripts/dna-parity/run.sh`, which needs the source checkout and so does not run in CI. | 09-23 |
| **D154** | **Knowledge tiers are defined, and four measurements about the DNA source are corrected** | **Tiers**, which D3, D47 and D79 used and nothing defined: **tier 0** is the grep blast radius (D47) and needs no store; **tier 1** is the evidence layer — findings (`RD-####`, each with `module` and `evidence`), the features that own them, and the derived service/surface/component of every finding — which is what a blast radius and a review reconciliation read; **tier 2** is the rest of the model — capabilities, domains, flows, stages, processes, ledgers and topology. The owner chose all layers (2026-09-23), so both tiers are built; the definition says which one a consumer depends on, and D152's line is the bar tier 1 must clear. **Corrections**, all measured on the canonical source (`plugins/tps-project-dna`, v2.18.1, commit `4a05eec`, 2026-09-09) rather than the v2.7.31 marketplace copy D84 and D107 read: (1) the Python is **8,900** lines, not 5,601, and `bootstrap_dd_sources.py`'s 2,495 are **28%** of it, not 45% — D107's conclusion (build in Node from the contract) stands and is stronger, since more of the source is contract and gates kiln ports rather than stack extractors it would not ship; (2) D84's template mismatch is **resolved upstream** — `export_static_explorer.py` now takes the skill's own `assets/explorer.html` and lets a project copy win only when one exists — so the vendor-time step is a hash check, not a rename; (3) D79's **"v1.2"** was tied to the Python dependency D107 removed and is withdrawn — DNA lands on the build that carries it; (4) **D3 is not superseded**: it permits Python behind the Knowledge port and nothing now uses that permission, so it stands unused rather than broken. | 09-23 |
| **D155** | **The bootstrap's density scan is a command, and what was scanned is recorded by blob** | The playbook's Phase 1 tells each project to *"write a density-ranking script per source repo … maintain the `ALREADY_SCANNED` ledger inside the script"*. A script per project is what a methodology can ask of its operator and what a product cannot ask of a stranger, so `kiln dna scan` is that script, written once: every tracked file of every repository (the project and each module, D81) **at its integration branch** — `origin/<branch>`, then the local branch, else `HEAD` marked as never pinnable (D80) — is classed OTHER (not code, vendored, generated, over 512 KB, or kiln's own `.kiln/`), TEST (by path), then CANDIDATE or SHELL by counting branch keywords, and the candidates are ranked by branches per line. **The ledger records the blob each path was read at** (`.kiln/dna/store/scan.json`), which is the one design change from the source's name list: a file whose content changed since it was read is unscanned again with nobody having to say so, a renamed file comes back as new (its findings still name the old path, so it has to), and a path the scanned commit no longer holds leaves the ledger, which is exactly the question `update`'s *"diff, don't re-scan"* asks (D22). `--out` writes one batch skeleton per group of files (never splitting a file across agents, ≤ 800 lines a group) carrying the commit each file is read at and **where to read it** — the working tree when it holds the same blob, otherwise a copy of the commit's blob under `.kiln/tmp/dna/`, because a finding describes the commit the ledger records and a feature branch checked out is not that commit. `kiln dna apply` then records the files at the blobs their commit holds (a file the commit lacks is refused) and moves `manifest.source_pins` only when that commit is still the integration branch's tip. **Review.** A reviewer who had not written it found two high defects, both reproduced: a tree entry named `..` — which git will fetch and clone, though not check out — made the snapshot write a file of the tree's choosing outside the project, and chunk ranges were counted in non-blank lines, so a third of a 1,000-line file was assigned to nobody and still recorded as scanned. Tree paths with `..`, `.`, `.git` or an absolute form are now dropped at the listing and the snapshot write is checked against its directory; ranges count physical lines. Also closed: a module that is not its own checkout (git answered for the parent, which was then pinned as the module) is refused; names with a tab or newline, and a file that became a directory, no longer break the listing or the blob comparison (the blob id is computed in Node, so no name passes through a pipe); snapshots are the blob's bytes, not a UTF-8 decoding; `--out` is resolved from the project root and compared by real path; and a malformed `scan` section or `--limit` is refused in words. | 09-23 |
| **D156** | **The infra map is drafted mechanically, and a stack file declares what counts as a surface** | The playbook's Phase 2b runs `bootstrap_infra_map.py` for a zero-token draft of services, surfaces and edges that a person reviews before anything consumes it. `kiln dna infra` keeps that shape — its output is a **batch for review**, never applied by the command, because naming a service is a decision that outlives the run — and changes where the guesses come from. **Services:** a module (D81) is a service; a single repository is one service whose roots are its code directories, split in two only where a directory is named like a front end; and a compose file's `image:` lines add the DATABASE / CACHE / QUEUE / SEARCH / GATEWAY services a project runs beside its code. This command's own first version clustered by first path segment, as the source does, and on kiln's own tree drafted six BACKEND services (`bin`, `hooks`, `lib`, …) for one CLI. **Surfaces:** the source minted SCREEN from directory names and its own field audit found 17 of 25 were widget libraries; kiln mints a surface only where a rule the stack file declares (`stacks/<id>.json` → `dna.surfaces`: a path pattern and, optionally, a content pattern) matches a file at the integration branch — Next.js pages and API routes, HTTP route registrations and cron schedules for `node`; view, JSON and CLI controllers and models for `php-ci3`. A new stack teaches the draft by adding rules, not code, which is D61's extension point doing the job D107 said `bootstrap_dd_sources.py`'s 2,495 stack-specific lines were doing. The same review found the first rules and service drafting too loose — an HTTP client call read as a route, `components/pages/` as a screen, a web controller guarded by `if (is_cli()) show_404()` as a batch, `webhooks/` as a front end, one database image drafted five times from five compose files, a metrics exporter as a database, and a project with modules drafting no service for its own code — and missing NestJS, SvelteKit, `.js` Next pages and CodeIgniter HMVC; each is now a test. Edge hints from finding prose are left out: the source measured its first version at 0 useful of 8, and hop-lift (D153) derives topology from cited call sites instead. | 09-23 |
| **D157** | **The DNA method ships as the source's own pages, and kiln's skill is the part that is kiln's** | "Knowledge layers: full, like the original" (owner, 2026-09-23) is a claim about the method as much as the store: the Identity litmus, the Flow acceptance test, the whole-inventory rule for Phase 5a, the rule-sentence bar and the claim-strength discipline are each the residue of a measured failure, and a paraphrase loses exactly the sentence that carried one. So nine reference pages of tps-project-dna v2.18.1 ship **verbatim** in `skills/kiln-dna/` under the D107 grant, each with a three-line note on top and nothing else changed, and `reading-note.md` maps every script and file they name to what it is in kiln — including, said plainly, the dozen scripts kiln does not ship and what to do instead. The superpowers fork (D69) shows the other cost: a copy whose instructions name tools that are not there fails silently at the moment it is followed; the note, and the rule that **kiln's commands win where a page disagrees**, is what keeps that from happening. `SKILL.md` is kiln's own: the playbook's phases mapped onto `kiln dna scan / infra / apply / check`, the three human gates (taxonomy, taxonomy extension, infra names), resume-by-reading-the-store, and five subagent prompts (scanner, clusterer, describer, process, auditor) that carry the dispatch rules `agent-orchestration.md` says every prompt must. The orchestrator hands `dna init` and `dna update` to it. **One gate family added with it:** the glossary requires the finding→feature relation to be *"exhaustive and exclusive"*, and the source enforced that in each round's throwaway merge script, not in either gate list; kiln now refuses a finding owned twice or an `rd_id` naming no finding on every write, and warns while findings remain unowned (a store mid-bootstrap has them). | 09-23 |
| **D158** | **Drift is measured against the ledger, fetched first, and asked at INVESTIGATE; code that is gone retires its findings** | D22 gave DNA one trigger — at the start of a ticket — and D82 required a fetch before the comparison; neither had a mechanism until now. `kiln dna drift` fetches `origin/<integration-branch>` for every repository (a remote-less project has nothing to fetch), then compares **the ledger's blobs with the tip's** rather than a commit range: a changed file, a new candidate and a deleted path are named with the findings each puts in doubt, and the size maps to D22's answers — `none`; `small` (≤ 20 files), catch up default yes; `large`, use the store with the drifted files low-confidence, default no — plus D82's `unknown` when a fetch failed, which is never printed as zero. The orchestrator runs it at INVESTIGATE whenever a store exists and puts the verdict first in the brief; catching up is the `kiln-dna` skill's round, on its own branch, never inside the work's diff. `kiln dna update` is the same measurement plus skeletons in which a changed file carries the blob it was read at and the findings citing it, so the scanner reads `git diff <was> <commit>:<path>` — the playbook's *"scan the diff, not the file"* — and it is refused on a store with no findings (D79's refinement 2, in the other direction). **One step the source lacks:** its update playbook only adds; nothing retires a finding whose code was deleted, so a map kept current by it only ever grows. Findings cannot be removed (id-schemes.md), so the skill moves them, in one batch, out of their features and into an `excluded` entry with `catalog: "RETIRED"` for the round — the contract unchanged, the finding still citable, and no feature claiming code that is gone. | 09-23 |
| **D159** | **Tier 1 answers beside tier 0, verified before use, and both are measured on the same work** | D152 set the bar: a richer tier that does not name more of what actually changed has not earned its cost. So `kiln blast` keeps the grep and, when a store with findings exists, adds tier 1 beside it rather than instead of it: a term reaches a file through a finding whose proposition names it **or through a feature whose name or description does**, which brings every file that feature's findings cite — the reach a grep lacks, since the words a ticket uses are the business's and the code's are not. Findings a round retired (D158) are skipped. **Verify before use** (Copilot Memory's rule, recorded in the OSS research for this plan): a named file whose blob differs from the one the store read is still named but marked `changed since the DNA read it`, and a missing one `gone`, so the agent opens it before relying on the claim. Both tiers are recorded on the work (`knowledge: [{tier: 0, …}, {tier: 1, …, changed}]`), and `kiln scope` and `kiln report` print one coverage line per tier — the comparison D47 asked the acceptance runs to make, now made on every work that has a store. `config.knowledge.tier`, which no code has ever read, is left as it was: the tier in use is the one the project has built, not one it declares. | 09-23 |
| **D160** | **The explorer is served on loopback behind a token, as it ships** | The owner's answer (2026-09-23, re-confirmed): a loopback server, not only an export. `kiln dna serve` listens on **127.0.0.1** only, on a free port, and every path carries a 128-bit random token, so neither another page in the browser nor another local process can read the store by guessing a port — the visual companion's model; a `Host` header other than `127.0.0.1:<port>` or `localhost:<port>` is refused, which closes DNS rebinding; it answers GET and HEAD of a fixed set of files (the vendored page, `laneflow.js`, and the contract's sixteen JSONL files plus `manifest.json` under the `_dna_store/` names the page asks for) and nothing else; and it stops itself after two hours with no request. **D84's vendor-time step, taken:** the page and its diagram engine are copied **unchanged** into `vendor/dna-explorer/` with their hashes in `NOTICE`, and the mismatch D84 recorded is gone upstream (D154). The source's own compatibility harness (`check_explorer_compat.py`) runs the page's loader under Node; kiln runs the same harness against a store kiln wrote, and the test fails unless both trees render with no console error — "it worked when I opened mine" is not a check, as the source says. **D83's allowlist, made mechanical:** the static egress test now permits `node:http` in exactly one file, `lib/dna/serve.mjs`, and separately asserts that file imports only `createServer`, binds `127.0.0.1` and opens no connection; the vendored page is checked to name no remote address; and the git-egress allowlist now reads `execFileSync("git", …)` calls too, which is where `kiln dna drift`'s `fetch` lives (D158) — before this change the check could not see it. **Review of D157–D160.** A reviewer who had not written them found ten defects, none high, each reproduced and now a test: the server died on an uncaught `ENOENT` when `kiln dna apply` swapped the store mid-request — the skill runs both at once — and now answers 503; a busy port surfaced as a kiln bug; the page's own Export button asked for `explorer-v2.html` at `/<token>/` and got a 404, now served as the page; the idle stop, 30 minutes on requests, closed a page still being read, now two hours; the clusterer's batch was refused because nothing counted an `excluded` id (kiln now counts `EXC-###`; id-schemes.md gives none) and because its flags rode in the batch (now a separate file); the scanner's diff command named the project path inside a module's repository (skeletons now carry `repo`, `repo_path` and `commit`); drift counted candidates an unfinished bootstrap had not read as new, which would have made INVESTIGATE report drift that was not there (now drift is what moved since the pin, and the leftovers are named separately); an unreadable store took tier 0 down with tier 1; `unknown` still printed its counts bare, now "as of the last fetch"; a fetch that would prompt now fails instead of hanging, and a timeout says so; blast showed excluded owners as features; the describer prompt claimed the gate refused words it does not check; the egress test's exemption also let `node:https` through and did not check every import. And, outside this range: a counter past 99 minted `DOM-X-01-100` and then failed its own id-shape gate — counters are minimum widths now. | 09-23 |
| **D161** | **A round states its exact size before it starts; a time figure comes only from this project's own recorded rounds** | The owner asked whether a DNA run could announce how long it will take. Measured against what mature agent tools do: none forecasts time or cost before a run — Claude Code and SWE-agent cap spend (`--max-budget-usd`, `per_instance_cost_limit`), Aider and OpenHands report it afterwards, the Agent SDK calls its own cost figure "a client-side estimate, not authoritative"; what they do prove is exact input counting and a hard cap. So `kiln dna scan` and `kiln dna update` print the exact size of the remaining work — files, lines, skeletons, waves of up to six subagents — and the skill shows it and asks for a cap (waves this session, or `--max-budget-usd` headless) before dispatching. A time figure appears only once the project has recorded rounds: each `INITIAL_BUILD` / `INCREMENTAL_RESCAN` record carries `metrics` (`lines_scanned`, `files_scanned`, `minutes`, and `cost_usd` only when the user read it from `/cost`), and the next round is estimated at that recorded pace, labelled as such. A first round gets no number, and says so — a forecast from one 8-file acceptance run would be invented precision, which AGENTS.md rules out. | 09-24 |
| **D162** | **Resume stays the ledger's job; a checkpoint ref guards against a wiped working tree** | Asked what happens when a bootstrap is cut off — a crash, a lost network, a plan limit. Compared with the systems built for it: LangGraph's checkpointer, dbt `retry`, SWE-agent's batch runs and the OpenAI Batch API all resume from a durable record of completed units and skip what it holds; kiln already has that shape — every `kiln dna apply` is atomic, the scan ledger records each file at its blob, and the skill resumes by reading the store — so it stays the resume path. What none of that covers is the store being removed from the working tree before a round is committed (`git clean`, a stash, a branch switch). Committing to the user's history for that would touch their index, HEAD and hooks; Cline's checkpoints solve the same risk with a shadow repository instead. kiln's version: after every successful apply the store is committed with `commit-tree` and a compare-and-swap `update-ref` to a checkpoint ref — local only, never pushed, never checked out; an unchanged store makes no new commit (the ref and the way the tree is built were corrected by D169). `kiln dna` names the checkpoint when the working tree has lost its store, and `kiln dna restore` writes it back, refusing to overwrite a store that is present. A checkpoint that cannot be taken never fails the write that already landed, and the apply says why it was not taken. | 09-24 |
| **D163** | **The project's domain dictionary is supported as the source defines it, and the store guard narrows to the store** | tps-project-dna's glossary defines a project-local `domain-dictionary-*.md`: the target system's business vocabulary, each standard term anchored to real source, its aliases, the concepts it is confused with, and ⚠ on terms agents must ask about rather than guess — optional, but **binding** on every agent when present. kiln had no place for it: D153's guard refused any write under `.kiln/dna/`. It now lives at `.kiln/dna/domain-dictionary-*.md`, beside the store, and is the one DNA file written by hand, because it records decisions about words rather than evidence; the guard keeps everything else under `.kiln/dna/` and lets through only `domain-dictionary-*.md` directly in it (a corpus row allows the dictionary with no work open; with a work open it is judged by the plan gate like any other file). The first version narrowed the guard to `.kiln/dna/store/` and opened the store's siblings; D169 closed that. The skill treats it as the source does — pasted into every scanner, clusterer, describer and process prompt, standard terms used, aliases resolved, ⚠ terms raised — and proposes starting one once a round has surfaced enough vocabulary. | 09-24 |
| **D164** | **Restructuring is a command, ported from the source's two restructure scripts** | Ids encode their parent (`DOM-X-NN-YY`, `BF-NN.S#.P#`), so moving one node renumbers everything beneath it and every reference to any of it. tps-project-dna does this with `apply_taxonomy_split.py` and `apply_stage_layout.py apply`, and each carries the defects that made it necessary: a first version that remapped only known fields and left 227 `related_feature_ids` dangling, a map applied twice that carried `DOM-DSH-04` through `DOM-DSH-03` to `DOM-PNV-01`, a scalar `legacy_*` that recorded only the first rename. D153 had pointed the skill at hand-written batches instead, which is the error-prone path those scripts exist to close. `kiln dna remap <plan.json> [--apply]` takes the source's plan schema (`new_domains`, `new_capabilities`, `capability_moves`, `feature_moves`, `deletes`, `restatements`) plus `stage_layouts` and `renumber_flows`, and keeps its rules: renumber every capability and feature in order under its parent; rewrite every exact old id in any field except a record's own id and its history (and ids in capability-axis prose); never rewrite a parent field already set to its new id; move a process's `primary_domain_id` with its capability; record `legacy_<kind>_id` and an `id_history` list; refuse a non-empty delete, an empty new capability, a node moved twice, a layout that does not cover its flow exactly, a non-bijective map or unconserved counts — and then the store's own gates, before anything is written. Dry run by default. Not ported: the `dossiers` and `sidecar` steps (kiln keeps no id-keyed sidecar) and the pre-mutation backups (the store is in git and every write is checkpointed, D162). | 09-24 |
| **D165** | **A feature's context footprint is counted by a port of the source's counter, and INVESTIGATE uses it for regression scope** | D159's tier 1 names files and the features that own them; it does not say which *other* features a change to those puts at risk, and "they share a file" alone is the signal the source's owner called too coarse — hub files make everything a neighbour. tps-project-dna's `context_footprint.py` (schema 3) answers exactly that, mechanically: per feature its rules, evidence files and processes, and neighbours by shared file, shared stage and typed relation, each feature-level neighbour tiered STRONG (evidence ranges overlap or within 50 lines), MEDIUM (same non-hub file, or the same process) or WEAK (a hub file cited by eight or more features, or a stage alone). `kiln dna footprint <feature-id ...> \| --all-planned` is that counter in Node and prints the same JSON; like the source it computes no hours. **Parity, measured:** over 300 generated stores — evidence both as plain strings and in kiln's `[{src, ref, loc}]` shape — it matches the Python exactly once two things are set aside that the source itself leaves undefined — which files a `sample` keeps above three, and which file a `tier_reason` names, both taken in Python set order, which changes with `PYTHONHASHSEED` (checked: two runs of the source over one store differ). kiln sorts there, so its output is stable; `scripts/dna-parity/run.sh` now runs this comparison too. One extension: the locator parser also reads `L3-L12`, the form kiln's scanner writes, which the source's pattern splits into two single lines. The orchestrator runs it at INVESTIGATE for the features tier 1 names and puts the STRONG and MEDIUM neighbours in the brief as regression scope. | 09-24 |
| **D166** | **Phase 5a's panel reconciliation is a command, ported from the source** | The whole-inventory journey synthesis is the bootstrap's most judgement-heavy step, and the source's answer for a large corpus is a panel: several independent agents propose the journey set, a script finds where they agree, and only the disagreements go to an adjudicator who reads the reasoning. Its lessons are built into `reconcile_5a.py`: panels emit a JSON manifest, because the first version parsed their prose and read a mention of another flow's process as an assignment; coverage is checked against the source roster, because three panels all dropped the same process while claiming full coverage; and agreement never decides — it only sorts what needs deciding. `kiln dna reconcile <manifest.json ...> --roster <sheet> [--min-agree n] [--out file]` is that script in Node: the same manifest schema, the same union-find over pairs a majority placed together, the same escalation rule (a cluster touched by more distinct flows than there are panels), the same result JSON — kiln adds only `schema_violations` to it, which the source prints to stderr. **Parity, measured:** 300 generated panel sets (184 with escalations, 11 with a process every panel dropped) give byte-identical results to the Python; `scripts/dna-parity/run.sh` runs it. The skill now runs 5a as a panel on a large corpus. Not ported: `build_panel_dossier.py`, which guards an *extract* of the methodology against truncation — kiln hands panels whole pages, so there is no extract to truncate. | 09-24 |
| **D167** | **The DNA method is a copy taken at a commit, and a script says what upstream has changed since** | D157 and D160 copied nine methodology pages and the explorer verbatim, and D153–D166 ported eight of the source's scripts. That is D69's position with superpowers — a fork, not a tracked pin — and it carries D69's cost: the source's later measured fixes do not arrive on their own. D69's compensation is a release-checklist step, read upstream from the fork point and adopt what is worth adopting as a decision entry, never as a merge; `docs/design/2026-09-23-upstream-read.md` shows it catching a real defect the fork was still carrying. `scripts/dna-upstream.mjs <claude_skill checkout>` is that step for DNA, mechanised as far as it honestly can be: the fork point is recorded as `METHODOLOGY_COMMIT` beside `METHODOLOGY_VERSION` (`lib/dna/contract.mjs`); the script counts the source's commits since it, compares each vendored page (minus kiln's three-line note) and each explorer asset with the checkout, and names every ported script that changed together with the kiln file that carries it — reading a local checkout only, no network. It exits non-zero on drift, so a release cannot miss it; what to adopt stays a judgement recorded as a decision. First run, 2026-09-24: the checkout stands at the fork commit, and every page and asset matches. | 09-24 |
| **D168** | **The DNA scanner runs on the mid tier, not the fast one** | The methodology puts "volume scanning" on the cheap tier and judgement on the strong one, and C8 followed it: a Haiku 4.5 scanner wrote 27 findings of which the orchestrator's audit corrected 18 and added 5 — thresholds restated wrongly, a flag described as the behaviour it only marked, a timezone invented. Those are reading-and-meaning errors, not transcription. Evidence from projects in use points the same way: Aider keeps its weak model to commit messages and summaries and never lets it read source for facts; superpowers sets the mid tier as the floor for any worker interpreting prose; the orchestrator/worker configurations in Anthropic's own published measurements — the multi-agent research system (https://www.anthropic.com/engineering/multi-agent-research-system) and the cost guide (https://platform.claude.com/docs/en/about-claude/models/optimizing-for-cost-and-intelligence) — use Sonnet workers; and the methodology's own clause applies — "when a stronger tier costs little at your call count, take it", which it does once the audit is otherwise a second extraction. Measured before deciding, on the same eight files and prompt (C9): a Sonnet scanner wrote 29, of which 3 were corrected and 1 added — 87% usable as written against 28%. The skill now dispatches the scanner with `model: "sonnet"` and keeps the fast tier for pure mechanical classification; clustering, panels and audits stay on the strongest tier. Two runs are two data points, recorded as such in `2026-09-24-acceptance-c9-dna-round2.md`. | 09-24 |
| **D169** | **The round-two review: the store's siblings, one checkpoint per project, and deletes that could re-point references** | A reviewer who had not written D161–D168 found three high defects, each reproduced. **(1)** Narrowing the guard to `.kiln/dna/store/` (D163) opened its siblings, and `store.previous` is what a read recovers when the store is missing: move the store away, copy it into `store.previous`, edit it, and the next apply adopted a record no gate had seen, with `kiln dna check` passing. The guard now covers all of `.kiln/dna/` except `domain-dictionary-*.md` directly in it, and the store and its directory join the removal anchors, so `rm -rf .kiln/dna` and `mv .kiln/dna/store …` are refused (six corpus rows). **(2)** One ref served the whole repository, so two kiln projects in a monorepo restored each other's store — force-rewriting the other's files — and a new worktree was offered the main worktree's. Each project now has its own ref under `refs/worktree/kiln/dna-checkpoint/<project path>`, per worktree by git's own rule; restore writes only entries under this project's store path, read with `--full-tree`. The tree is built from `hash-object --no-filters` and `mktree` rather than `git add`, so the user's clean filters and fsmonitor never run, and `update-ref` runs with hooks off, so a `reference-transaction` hook neither sees nor blocks it — D162 had claimed the hooks were untouched when they were not. **(3)** A deleted capability is in no old-to-new map and its number goes to the next capability, so an excluded entry or a typed edge still naming it silently named another; the source has the same hole. kiln now judges emptiness on the store as it was (the source's rule — a plan may not empty and delete in one step) and refuses a delete while anything still names it. Also closed: remap no longer rewrites ledger records (an update's remap table is history, as id-schemes.md says), process fields it rewrites keep a `legacy_<field>` copy as the source's pass does, a stageless process is named rather than reported as a count mismatch, `--min-agree` and an empty or repeated roster behave as the source's, `--limit` shows what this run takes, a metric that is not a number is not a measurement, and `dna-upstream.mjs` reports every upstream change it does not carry. The parity harness now also generates kiln-shaped evidence; all three comparisons stay at 0 of 300. | 09-24 |
| **D170** | **Third-party code is left out of the DNA scan by GitHub Linguist's attributes** | The first real bootstrap (a CodeIgniter 3 multi-repo) listed 3155 candidates, and 1555 of them — 425k of 849k lines — were the framework's `system/`, bundled SDKs and PDF libraries the built-in path patterns do not name; the scan ranks densest first, so they came first, and with no way to exclude them the scan could never report itself exhausted. Linguist, which GitHub runs on every repository, already answers this with two gitattributes: `linguist-vendored` and `linguist-generated` leave a path out (any value but `false`, as Linguist's `boolean_attribute` reads it), `-linguist-vendored` keeps one its `vendor.yml` patterns would drop. kiln asks git for them (`git check-attr -z --stdin`, no parser of its own), from the checkout rather than the scanned commit, so `.git/info/attributes` works for a team that will not commit the lines; the built-in patterns decide only where the attributes are silent, and `.kiln/` stays out regardless. Linguist's `-min.js` pattern joins `.min.js`. `kiln dna scan` counts what is left out and says how to mark more, and the skill tells the agent to ask for the marks instead of recording empty skeletons. | 09-25 |
| **D171** | **`kiln doctor` names an integration branch a checkout does not have** | In the same bootstrap `kiln doctor` reported Ready, and `kiln dna scan` only later found the superproject had no `v3-master` — neither on a remote nor locally — so the store could not be pinned (D80); the check printed the configured branch and never asked git. `git flow init` verifies its branches exist before it accepts them; doctor now does the same for every checkout, with the refs `scanRef` uses (`origin/<branch>`, then the local one), and warns with the checkout's path and the D81 per-module map as the fix. A checkout with no commit yet, or one that is not there, is not judged; a module directory that is not a checkout of its own — never initialised, so git answers for the repository above it — is named as that rather than judged by its parent's branches. The warning keeps each checkout's configured branch in view. The duplicated, outdated comment above the check is gone. | 09-25 |
| **D172** | **`${ref}` in `branch_pattern` is the tracker's ref, and a branch name git refuses is named at ship** | The same project's convention is `feature/v3/#<ticket>`, and the pattern offered only `${id}`, `${slug}` and `${type}`: a ticket's work id is `<module>-<ref>` (D51), so the nearest it could render was `feature/v3/admin-page-1586`. GitLab's branch-name template answers this with `%{id}`, the issue's own number, and `gh issue develop` names branches the same way; kiln adds `${ref}` with that meaning — `1586` from `admin-page-1586`, `SE-5236` from `SE-5236.2` (a follow-up renders its ticket's ref, as GitLab's `%{id}` would), and nothing from a sentence's id, which the existing unfillable-placeholder rule then names. Because a pattern can now carry characters such as `#`, the rendered name is checked with `git check-ref-format --branch` (which also refuses `-foo` and `HEAD`, legal ref names but not branches) and one git refuses is reported by `kiln ship` rather than by `git switch`. | 09-25 |
| **D173** | **At init the agent offers only commands that run as written** | At the same setup the agent itself proposed `make test NO_DUMP=1 FILTER=...` as the recommended fast command — `init --propose` had offered none — and only caught, one question later, that kiln runs `stack.cmd.test_fast` verbatim, so the literal `...` would have reached PHPUnit as a filter matching nearly every test. superpowers' verification-before-completion holds that a claim needs fresh evidence first; a recommended command is such a claim. The orchestrator's init section now says so: offer kiln's own answer or a command just run and seen pass, mark nothing else recommended, and leave the fast command blank when it needs a per-task argument. A per-task placeholder was not added — nothing yet needs one. | 09-25 |
| **D174** | **A citation is checked where it is written: `ref` relative to its repository, and the file and lines must exist** | On the first real bootstrap, scanners wrote `ref` two ways — `AdminPage/application/x.php` and `application/x.php` — and `loc` as `L311-317`; `kiln dna apply` accepted every form, and only the agent's own audit script caught them. Reproduced: a 6-line file accepted `ref: "price.js"` (no such file), `L2-9` and `L400-L420`. The fault was kiln's: tps-project-dna makes `ref` relative to the evidence source `src` names (dna-store.md: "source-relative", `src` an `evidence_sources` id), as SARIF resolves `uri` against `uriBaseId` and GitHub code scanning takes paths from the repository root — and the source's own changelog records the same drift (Tradom's two bases) patched by a heuristic. kiln's skeleton handed scanners `repo` as a path and the prompt said `ref: <path>`, the project path. The skeleton now carries `src` (the repository's name, the id `kiln dna infra` declares) and `ref` (the path inside it), and the prompt says to copy them. `apply` refuses, naming each record: a finding's citation that is not an object with `src` and `ref` (without `src` nothing else could be checked), a `src` that is neither a repository nor a declared evidence source, and — for code — a `loc` that is not `L<n>` or `L<n>-L<m>`, one that runs backwards (SARIF1007, an error there too), a `ref` its repository does not hold at the round's commit, or a range past the file's last line. A document source keeps its own kind of anchor. A retired finding is changed by id without its evidence, since its file is gone. The last two go beyond SARIF and the source, which check neither; the writers here are models, and the source's quality gates record a wrong-file citation only a sampled audit caught. Nothing is rewritten silently; a `ref` carrying its repository's directory is told to drop it. Records already in a store are not re-judged: a store written before this re-cites them through a batch. | 09-25 |
| **D175** | **`kiln dna apply` refuses a record that carries a credential** | The store is committed, and a finding's `quote` is copied code. On the first real bootstrap two scanned files held AWS keys in source; the scanners happened not to copy them, and nothing would have stopped it — reproduced: a quote holding an `AKIA…` key was written. tps-project-dna says nothing about credentials in evidence. gitleaks (MIT) and detect-secrets (Apache-2.0) are the proven scanners, and GitHub push protection blocks only "the most recent token versions that secret scanning can identify with confidence" to avoid false positives. kiln takes gitleaks' ten prefix-anchored rules — AWS access key id, private key, GitHub PAT/fine-grained/OAuth, Slack bot/user/webhook, Stripe, GCP API key — recorded in NOTICE with the commit, and runs them over every string of every record a batch upserts and of its settings — each string as written, since JSON's escaping puts `\n` before a key and `\"` after one and defeats the rules' anchors (the first version tested the serialised record and let a double-quoted key through). The AWS rule keeps gitleaks' allowlist for keys ending in `EXAMPLE`, AWS's documentation keys. It blocks, as push protection does, naming the record and the rule and never the value, with no bypass flag. Left out: gitleaks' generic keyword rule and entropy-only detectors (the noisy ones), and secrets with no prefix — an AWS secret access key is found only by detect-secrets' proximity heuristic, which is low confidence; the scanner prompt instead forbids quoting any credential value. | 09-25 |
| **D176** | **`kiln dna infra` drafts `components.custom` for the person to confirm** | On the first real bootstrap every component of a project that wrote all its own code came out `origin: VENDOR`. tps-project-dna makes `components.custom` the component tier's one authored input and reads its absence as VENDOR — "the honest reading for a store built from a vendored platform" — measured on an Odoo fork (586 components, 2 custom) whose owner wrote the list; no script drafts it and no step asks, so on any other project the default is backwards and stays so. Linguist, Renovate and SonarQube all take the opposite default: code is the project's own unless it is marked or matched as someone else's. kiln keeps the source's rule that origin is authored and follows its own pattern for such decisions ("system drafts, BA confirms", bpm-alignment.md): `kiln dna infra` puts the store's PROJECT components — the kind the source's glossary defines as "a component the project owns" — in `settings.components.custom` of the draft the user already reviews at Phase 2b, and leaves ADDON and FRAMEWORK roots for them to add. A list already declared is theirs and is not redrafted; a PROJECT root a later round finds is named by `kiln dna infra` as read VENDOR, for them to add. This departs from the source by drafting at all. | 09-25 |
| **D177** | **Every applied scan round is timed, so an interrupted bootstrap still has a pace** | D161 took pace only from `metrics` on update records, which a bootstrap writes at Phase 8, its end. The first real bootstrap stopped after three waves (60 files, about 10.6 min) with 53 waves to go, so the next session could say nothing, and the agent did the arithmetic itself — what D161 exists to prevent. tps-project-dna records no timing at any level. dbt's `run_results.json` is the proven shape: each completed unit's `execution_time` kept on disk; only a live progress bar (tqdm) extrapolates from it. kiln now does the same per round: `kiln dna scan --out` and `kiln dna update --out` stamp `scan.started` on the skeletons they write, and `apply` appends `{started, finished, files, lines}` to a `rounds` list in `scan.json` — once per skeleton, so applying one again does not count its work twice —, kiln's own ledger beside the path-to-blob map, so no contract field changes. The skeletons of one round are read side by side, so a round's time is its last apply less its start, not the sum of each skeleton's. A start in the future or more than a day back measures a pause and is not recorded. Timed rounds are the pace; an update's `metrics` spans rounds already among them, so it is read only for a store that has none, and still supplies cost, which only a person reading `/cost` knows. Unchanged from D161: no figure until something is recorded, and the figure is labelled as past rounds, not a promise. The stamp is kiln's, not the agent's clock, so the one caveat — skeletons written well before dispatch count the wait — is stated in the skill. | 09-25 |
...
- ~~D98's inline-program flags `-c`, `-e`, `-r`~~ → widened by **D129** to the question the flags
  stood for.
- ~~D79's "shipped at v1.2" and B12's reservation message~~ → withdrawn by **D154** and **D153**.
  The version was tied to the Python dependency D107 removed; `kiln dna` is a command now.
- ~~D84's template-name mismatch and D107's 5,601 lines / 45%~~ → corrected by **D154**, measured
  on the canonical v2.18.1 source instead of the stale v2.7.31 copy.
