...
| **The verbatim request** — *not needed* | BMAD's intent auditor has an approved intent; kiln's paraphrase is weaker for the raw ask | Resolved by reading the source: BMAD's `{verbatim_intent}` is the plan's `## Intent` section, written by the agent, approved by the human and frozen after approval (`bmad-build/step-oneshot.md`, `plan-template.md`) — not a capture of the raw request. kiln's intent auditor already gets the brief and the plan's goal, and the plan gate binds that plan by hash (D139). |
